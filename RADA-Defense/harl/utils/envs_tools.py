    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "smac":
                ...
                if env_args.get("scenario", "") in ["simple_tag_v2", "simple_tag_v3"]:
                    from harl.envs.pettingzoo_mpe.simple_tag_env import (
                        SimpleTagAdversaryEnv,
                    )
                    env = SimpleTagAdversaryEnv(env_args)
                else:
...
def make_render_env(env_name, seed, env_args):
    ...
    if env_name == "smac":
        ...
        if env_args.get("scenario", "") in ["simple_tag_v2", "simple_tag_v3"]:
            ...
            env = SimpleTagAdversaryEnv({**env_args, "render_mode": "human"})
        else:
