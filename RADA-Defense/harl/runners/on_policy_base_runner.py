...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if self.algo_args["render"]["use_render"] is False:  # train, not render
            ...
            # ===== [ReCL] Initialization START =====
            self.use_recl = algo_args["algo"].get("use_recl", False)
            if self.use_recl:
                # Derive obs_dim from the first agent's observation space
                obs_space_0 = self.envs.observation_space[0]
                recl_obs_dim = obs_space_0.shape[0]

                self.recl = ReCL(
                    obs_dim=recl_obs_dim,
                    num_agents=self.num_agents,
                    agent_embedding_dim=algo_args["algo"].get("recl_agent_embedding_dim", 64),
                    role_embedding_dim=algo_args["algo"].get("recl_role_embedding_dim", 32),
                    cluster_num=algo_args["algo"].get("recl_cluster_num", 2),
                    cl_lr=algo_args["algo"].get("recl_cl_lr", 1e-3),
                    agent_embedding_lr=algo_args["algo"].get("recl_agent_embedding_lr", 1e-3),
                    tau=algo_args["algo"].get("recl_tau", 0.005),
                    multi_steps=algo_args["algo"].get("recl_multi_steps", 10),
                    cluster_once_per_episode=algo_args["algo"].get("recl_cluster_once", True),  # 新增
                    skip_ae_loss=algo_args["algo"].get("recl_skip_ae_loss", False),  # 新增
                    device=self.device,
                )
                self.recl_train_freq = algo_args["algo"].get("recl_train_freq", 1)
                self.recl_train_step = 0
                print(f"[ReCL] Initialized: obs_dim={recl_obs_dim}, "
                      f"num_agents={self.num_agents}, "
                      f"clusters={algo_args['algo'].get('recl_cluster_num', 2)}")
            # ===== [ReCL] Initialization END =====
    ...
    def _gather_obs_for_recl(self):
        obs_list = []
        active_list = []
        for agent_id in range(self.num_agents):
            # actor_buffer[i].obs shape: (episode_length+1, n_threads, *obs_shape)
            obs_list.append(self.actor_buffer[agent_id].obs)
            # actor_buffer[i].active_masks shape: (episode_length+1, n_threads, 1)
            active_list.append(self.actor_buffer[agent_id].active_masks)

        # Stack along a new agent axis → (ep_len+1, n_threads, N, obs_dim)
        batch_obs = np.stack(obs_list, axis=2)
        # Transpose → (n_threads, ep_len+1, N, obs_dim)
        batch_obs = batch_obs.transpose(1, 0, 2, 3)

        # (ep_len+1, n_threads, N, 1) → squeeze last dim → (ep_len+1, n_threads, N)
        batch_active = np.stack(active_list, axis=2).squeeze(-1)
        # Transpose → (n_threads, ep_len+1, N)
        batch_active = batch_active.transpose(1, 0, 2)

        return batch_obs, batch_active
    ...
    def save(self):
        ...
        if hasattr(self, "use_recl") and self.use_recl:
            self.recl.save(str(self.save_dir) + "/recl.pt")
        ...
        if not self.algo_args["render"]["use_render"]:
            ...
            if self.value_normalizer is not None:
                ...
                try:
                    ...
                    print(f"[Warning] value_normalizer load failed: {e}")
                    print("[Warning] Attempting strict=False ...")
                    self.value_normalizer.load_state_dict(value_normalizer_state_dict, strict=False)
            # ===== [ReCL] Restore START =====
            if hasattr(self, "use_recl") and self.use_recl:
                recl_path = (
                    str(self.algo_args["train"]["model_dir"]) + "/recl.pt"
                )
                import os
                if os.path.exists(recl_path):
                    self.recl.load(recl_path)
                    print(f"[ReCL] Restored from {recl_path}")
            # ===== [ReCL] Restore END =====
